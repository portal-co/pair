...
use relooper::{ShapedBlock, RelooperLabel};
...
pub trait UnTreeTerminator<M: ModLike<Fun = F>, F: FunLike<Terminator = Self>, Err>: TreeTerminator<M,F,Err>{
    fn get_tree(&self, n: &F) -> Result<Tree<M,F>,Err>;
}
pub enum Tree<M: ModLike<Fun = F>, F: FunLike>{
    Just(Entry<M>),
    Switch(ValIDFun<F>, Vec<Entry<M>>, Entry<M>)
}
pub trait Reloop<M: ModLike<Fun = Self>,Err>: FunLike + Sized where Self::Terminator: UnTreeTerminator<M,Self,Err>, FunId<M>: RelooperLabel{
    fn reloop(m: &M, i: &FunId<M>) -> Result<Box<ShapedBlock<FunId<M>>>,Err>{
        let mut n = BTreeMap::new();
        Self::collect(m, i, &mut n)?;
        return Ok(relooper::reloop(n.into_iter().collect(), *i));
    }
    fn collect(m: &M, i: &FunId<M>, go: &mut BTreeMap<FunId<M>,Vec<FunId<M>>>) -> Result<(),Err>;
}
impl<T: FunLike,M: ModLike<Fun = T>,Err> Reloop<M,Err> for T where Self::Terminator: UnTreeTerminator<M,Self,Err>, FunId<M>: RelooperLabel{
    fn collect(m: &M, i: &FunId<M>, go: &mut BTreeMap<FunId<M>,Vec<FunId<M>>>) -> Result<(),Err> {
